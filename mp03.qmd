...
-   Data Acquisition — Downloading tree, district, risk, and maintenance data
-   Data Integration — Spatial joining and processing
-   Initial Explorations — District-level tree analysis
-   Government Project — District 2 tree health initiative proposal
:::
...
\
🌳 **Total Trees:** `r format(d2$total_trees, big.mark = ",")`

💀 **Dead Trees:** `r format(d2$dead_trees, big.mark = ",")` (`r sprintf("%.2f%%", d2$percent_dead)`)

⚠️ **Critical Condition:** `r format(d2$critical_trees, big.mark = ",")` (`r sprintf("%.2f%%", (d2$critical_trees/d2$total_trees)*100)`)

⚡ **Poor Condition:** `r format(d2$poor_trees, big.mark = ",")` (`r sprintf("%.2f%%", (d2$poor_trees/d2$total_trees)*100)`)

❌ **Total Unhealthy:** `r format(d2$unhealthy_trees, big.mark = ",")` (`r sprintf("%.2f%%", d2$percent_unhealthy)`)
:::
...
-   💀 **`r format(d2$dead_trees, big.mark = ",")`** dead trees requiring immediate removal

-   ⚠️ **`r format(d2$critical_trees, big.mark = ",")`** critical condition trees needing urgent treatment
...
-   📋 **Only `r format(sum(d2_maintenance$citycouncil == "2.0", na.rm = TRUE), big.mark = ",")`** active work orders for District 2

-   🪓 **`r format(sum(d2_maintenance$wotype == "Tree Removal" & d2_maintenance$citycouncil == "2.0", na.rm = TRUE), big.mark = ",")`** tree removal orders (covers only `r sprintf("%.0f%%", (sum(d2_maintenance$wotype == "Tree Removal" & d2_maintenance$citycouncil == "2.0", na.rm = TRUE) / d2$dead_trees) * 100)` of dead trees)

-   🌳 **`r format(sum(d2_maintenance$wotype == "Stump Removal" & d2_maintenance$citycouncil == "2.0", na.rm = TRUE), big.mark = ",")`** stump removal orders (blocking new plantings)
\
