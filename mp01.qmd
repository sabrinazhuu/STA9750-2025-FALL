...
**Data Period:** 2020 – Septembver 2025
...
# Display with datatable - only selected columns
all_countries_data |>
...
# Press Releases {style="color:#3255A;"}
...
# Display as an interactive datatable with conditional formatting
datatable(
  india_top10_films,
  ...
  backgroundColor = styleEqual(
    india_top10_films$show_title,
    ifelse(india_top10_films$show_title == "Red Notice", "white", "#fff2d9")
  )
)
```
...
# Step 4: Get detailed info with total weeks for India-only programs
...
# Table
# The titles you want to highlight
highlight_titles <- c(
...
# Find the 0-based row indices for those titles in your table
highlight_rows <- which(india_only_top$show_title %in% highlight_titles) - 1

# Datatable with only those rows highlighted
datatable(
  india_only_top,
  options = list(
    dom = "t",
...
# Create datatable with row highlighting
datatable(
