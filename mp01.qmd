...
- **Part 1:** Data Preparation — Importing, cleaning, and organizing Netflix's Top 10 data.
...
datatable(
  ...
  formatStyle(
    "show_title",
    target = "row",
    backgroundColor = styleEqual(highlight_titles, rep("#fff2d9", length(highlight_titles))),
    fontWeight = styleEqual(highlight_titles, rep("bold", length(highlight_titles)))
  )
```
...
datatable(
  ...
  formatStyle(
    "show_title",
    target = "row",
    backgroundColor = styleEqual(highlight_titles, rep("#ebfaec", length(highlight_titles))),
    fontWeight = styleEqual(highlight_titles, rep("bold", length(highlight_titles)))
  )
```
...
```{r}
# Table for films
top_film <- top3_films$show_title[1]

datatable(
  top3_films |>
    select(Film = show_title, `Total Views (Millions)` = views_millions),
  options = list(
    dom = "t",
    searching = FALSE,
    info = FALSE
  ...
  formatStyle(
    "Film",
    target = "row",
    backgroundColor = styleEqual(top_film, "#dbe5ff"),
    fontWeight = styleEqual(top_film, "bold")
  )

# Table for TV shows
top_tv_show <- top3_tv$show_title[1]

datatable(
  top3_tv |>
    select(`TV Show` = show_title, `Total Views (Millions)` = views_millions),
  options = list(
    dom = "t",
    searching = FALSE,
    info = FALSE
  ...
  formatStyle(
    "TV Show",
    target = "row",
    backgroundColor = styleEqual(top_tv_show, "#dbe5ff"),
    fontWeight = styleEqual(top_tv_show, "bold")
  )
```
\
...
datatable(
  ...
  formatStyle(
    "show_title",
    target = "row",
    backgroundColor = styleEqual(highlighted_shows, rep("#dbe5ff", length(highlighted_shows))),
    fontWeight = styleEqual(highlighted_shows, rep("bold", length(highlighted_shows)))
  )
```
\
